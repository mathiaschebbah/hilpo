...
from milpo.inference import ApiCallLog, PostInput, async_classify_alma_batch
...
def _make_classify_post_mock(slow_ids: set[int], delay: float = 10.0):
    """Crée un mock qui hang pour les posts dans slow_ids, instantané pour les autres."""
    ...
    async def _mock(
        ...
        return PipelineResult(
            prediction=pred,
            api_calls=[ApiCallLog("descriptor", "gemini", 100, 1000, 100)],
        )
...
class PostTimeoutTests(unittest.IsolatedAsyncioTestCase):
    """async_classify_alma_batch doit supporter un per_post_timeout."""
    ...
    async def test_fast_posts_returned_when_one_post_hangs(self) -> None:
        ...
        with patch("milpo.inference.async_classify_post_alma", new=mock):
    ...
    async def test_on_progress_called_for_timed_out_posts(self) -> None:
        ...
        with patch("milpo.inference.async_classify_post_alma", new=mock):
            await async_classify_alma_batch(
                posts=posts,
                labels_by_scope=_LABELS,
        ...
        self.assertEqual(final[2], 1)  # errors (le post timed out)
    ...
    async def test_exception_in_one_post_does_not_kill_others(self) -> None:
        ...
        with patch("milpo.inference.async_classify_post_alma", new=_mock):
            results = await async_classify_alma_batch(
                posts=posts,
                labels_by_scope=_LABELS,
                per_post_timeout=5.0,
