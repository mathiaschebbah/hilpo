...
- **État des phases** : Phase 1 ✅ (infra annotation + split test annoté intégralement, annotation dev en cours — volumes à jour accessibles en BDD via `/setup`). Phase 2 ✅ — pipeline descripteur (Gemini 3 Flash Preview) + 3 classifieurs (Qwen 3.5 Flash + tool calling), **B0 stabilisé : 85,4% / 73,9% / 95,9% (run id=23, 437/437 posts)**, prompts v0 lockés en BDD via la migration `[006_seed_prompts_v0.sql](../apps/backend/migrations/006_seed_prompts_v0.sql)`. Phase 3 ✅ implémentée côté code — boucle ProTeGi fidèle (gradient textuel LLM_∇ + édition LLM_δ + paraphrase monte-carlo LLM_mc + Successive Rejects), simulation prequential, gradients et candidats persistés en BDD via la migration `[008_protegi_loop.sql](../apps/backend/migrations/008_protegi_loop.sql)`. Run complet sur le dev en attente de la fin des annotations.